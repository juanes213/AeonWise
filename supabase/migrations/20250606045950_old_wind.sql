/*
  ...
  1. Courses
    - Adds initial course catalog with 6 courses
    - Includes various difficulty levels and categories
    - Each course has a title, description, duration, and modules

  2. Mentorship Profiles
    - Creates mentorship profiles for existing users
    - Includes specialties, pricing, and availability
    - Each profile has a unique IPFS hash for additional data
*/
...
-- Create mentorship profiles only if the user exists
DO $$
DECLARE
  user_id1 uuid;
  user_id2 uuid;
...
BEGIN
  ...
  SELECT id INTO user_id1 FROM profiles WHERE username = 'Alexandria' LIMIT 1;
  SELECT id INTO user_id2 FROM profiles WHERE username = 'Marcus' LIMIT 1;
  SELECT id INTO user_id3 FROM profiles WHERE username = 'Sophia' LIMIT 1;
  SELECT id INTO user_id4 FROM profiles WHERE username = 'Julian' LIMIT 1;
  SELECT id INTO user_id5 FROM profiles WHERE username = 'Elena' LIMIT 1;
  SELECT id INTO user_id6 FROM profiles WHERE username = 'Raj' LIMIT 1;
